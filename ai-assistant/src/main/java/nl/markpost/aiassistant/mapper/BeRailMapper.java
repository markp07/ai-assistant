...
public class BeRailMapper {
  ...
  public Departure mapDeparture(String station,
    ...
    List<@Valid CompositionResponseCompositionSegmentsSegmentInner> segments = Optional.ofNullable(
            compositionResponse.getComposition())
        .map(composition -> composition.getSegments().getSegment())
        .orElse(List.of());

    departure.setTrainNumber(Optional.ofNullable(beRailDeparture.getVehicleinfo())
        .map(vehicleInfo -> vehicleInfo.getShortname().split(" ")[1])
        .orElse(null));
    departure.setDestination(Optional.ofNullable(beRailDeparture.getStationinfo())
        .map(LiveboardResponseDeparturesDepartureInnerStationinfo::getStandardname)
        .orElse(null));
    departure.setDepartureTime(Optional.ofNullable(beRailDeparture.getTime())
        .map(time -> OffsetDateTime.ofInstant(Instant.ofEpochSecond(time.longValue()),
            ZoneOffset.ofHours(1)))
        .orElse(null));
    departure.setDelay(Optional.ofNullable(beRailDeparture.getDelay())
        .map(delay -> delay.intValue() / 60)
        .orElse(null));
    departure.setTrack(beRailDeparture.getPlatform());
    departure.setCancelled(Optional.ofNullable(beRailDeparture.getCanceled())
        .map(canceled -> canceled == 1)
        .orElse(false));
    departure.setCategory(Optional.ofNullable(beRailDeparture.getVehicleinfo())
        .map(vehicleInfo -> {
    ...
    departure.setVia(Optional.ofNullable(vehicleResponse.getStops())
        .map(stops -> {
          ...
          return stops.getStop().stream()
              .filter(stop -> {
                ...
                if (stop.getStationinfo().getStandardname().equals(station.replace("%20", "-"))) {
    ...
    departure.setTrainLength(segments.stream()
    ...
    departure.setNumberOfSeats(segments.stream()
        ...
        .reduce(0, Integer::sum));
    departure.setDepartureStatus(Optional.ofNullable(vehicleResponse.getStops())
        .map(stops -> stops.getStop().stream()
            .filter(
                stop -> stop.getStationinfo().getStandardname().equals(station.replace("%20", "-")))
            .findFirst()
            .map(stop -> {
